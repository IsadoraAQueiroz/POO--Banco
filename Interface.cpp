...
{
    dataInv = dataInv.substr(6,9) + dataInv.substr(2,4)+ "/"
                + dataInv.substr(0,1);
}
...
bool Interface::Menu(Banco& aberto)
{
	...
	switch (escolha)
	{
		case 'a': NovoCliente(aberto); break;
		case 'A': NovoCliente(aberto); break;
		case 'b': NovaConta(aberto); break;
		case 'B': NovaConta(aberto); break;
		case 'c': ExcluirCliente(aberto); break;
		case 'C': ExcluirCliente(aberto); break;
		case 'd': ExcluirConta(aberto); break;
		case 'D': ExcluirConta(aberto); break;
		case 'e': Deposito(aberto); break;
		case 'E': Deposito(aberto); break;
		case 'f': Saque(aberto); break;
		case 'F': Saque(aberto); break;
		case 'g': Transferencia(aberto); break;
		case 'G': Transferencia(aberto); break;
		case 'h': Tarifa(aberto); break;
		case 'H': Tarifa(aberto); break;
		case 'i': CPMF(aberto); break;
		case 'I': CPMF(aberto); break;
		case 'j': Saldo(aberto); break;
		case 'J': Saldo(aberto); break;
		case 'k': Extrato(aberto); break;
		case 'K': Extrato(aberto); break;
		case 'l': Clientes(aberto); break;
		case 'L': Clientes(aberto); break;
		case 'm': Contas(aberto); break;
		case 'M': Contas(aberto); break;
		case '*' : break;
...
{
    cout << "a ser excluida: "
	aberto.excluiConta(obter_nConta());
}

void Interface::Deposito(Banco& aberto)
{
    cout << "a ser depositada: "
    nConta = obter_nConta;
...
void Interface::Extrato(Banco& aberto)
{
    ...
    switch (escolha)
	{
		case 1:
		    string inicio, fim;
		    cout << "Digite a data inicial(formato dd/mm/aaaa): "
		    cin >> inicio;
		    cout << "Digite a data final(formato dd/mm/aaaa): "
		    cin >> fim;
		    inv_data(inicio);
		    inv_data(fim);
		...
		case 3:
		    ...
		    break;
        case 4 : break;
        default: cout << "Voce nao digitou um numero que corresponda ao menu. Digite novamente." << endl; Extrato(aberto); break;
}
...
void Interface::Clientes(Banco& aberto)
{
    ...
    while (i!=lista_clientes.end()){
        ...
        cout  << "\tTel: "  << i -> Getfone() << endl << i -> Getendereco() << endl;
    }
}
...
void Interface::Contas(Banco& aberto)
{
    ...
    while (i!=lista_contas.end()){
        ...
        cout << "Conta: " << i -> getNumConta() << "\t" << "Saldo: " << i -> getSaldo()
    }
}
...
int Interface:: main()
{
    ...
    while(Menu(aberto))
        cout << "A operacao foi finalizada. O menu sera apresentado novamente. Para sair do programa, selecione a opcao ""sair do programa"" no menu. \n";
